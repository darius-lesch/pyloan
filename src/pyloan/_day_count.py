...
def _get_julian_day_number(y, m, d):
    return (1461 * (y + 4800 + (m - 14) / 12)) / 4 + (367 * (m - 2 - 12 * ((m - 14) / 12))) / 12 - (
                3 * ((y + 4900 + (m - 14) / 12) / 100)) / 4 + d - 32075


def _thirty_a_360(dt1, dt2, eom=False):
...
def _thirty_u_360(dt1, dt2, eom=False):
    y1, m1, d1 = dt1.year, dt1.month, dt1.day
    y2, m2, d2 = dt2.year, dt2.month, dt2.day
...
def _thirty_e_360(dt1, dt2, eom=False):
    y1, m1, d1 = dt1.year, dt1.month, dt1.day
    y2, m2, d2 = dt2.year, dt2.month, dt2.day
...
def _thirty_e_360_isda(dt1, dt2, eom=False):
    y1, m1, d1 = dt1.year, dt1.month, dt1.day
    y2, m2, d2 = dt2.year, dt2.month, dt2.day
    dt1_eom_day = cal.monthrange(y1, m1)[1]
    dt2_eom_day = cal.monthrange(y2, m2)[1]
...
def _a_365f(dt1, dt2, eom=False):
    return (dt2 - dt1).days, 365


def _a_360(dt1, dt2, eom=False):
    return (dt2 - dt1).days, 360


def _a_a_isda(dt1, dt2, eom=False):
    y1, m1, d1 = dt1.year, dt1.month, dt1.day
    y2, m2, d2 = dt2.year, dt2.month, dt2.day
...
def _a_a_afb(dt1, dt2, eom=False):
    y1, m1, d1 = dt1.year, dt1.month, dt1.day
    y2, m2, d2 = dt2.year, dt2.month, dt2.day
